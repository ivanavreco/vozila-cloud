...
if (!class_exists('Newscrunch_About_Page')) {
	class Newscrunch_About_Page {
		...
		public function add_theme_info_menu() {
			...
			$title = sprintf(esc_html__('About %1$s %2$s', 'newscrunch' ), esc_html($theme->get('Name')), $count);
			/* translators: %1$s: theme name and %2$s: version of the theme*/
			add_menu_page(sprintf(esc_html__('Welcome to %1$s %2$s', 'newscrunch' ), esc_html($theme->get('Name')), esc_html($theme->get('Version'))), $title, 'edit_theme_options', 'newscrunch-welcome', array($this, 'print_welcome_page'));	
		}
		...
		public function welcome_admin_notice() {
			...
			<div class="updated notice is-dismissible">
				...
				printf( esc_html__('%1$s theme is installed. To take full advantage of the features this theme has to offer visit our %2$swelcome page%3$s', 'newscrunch'), esc_html($theme_name), '<a href="' . esc_url( admin_url( 'admin.php?page=newscrunch-welcome' ) ) . '" class="newscrunch-wl-page-link">', '</a>' ); ?></p>
				...
				printf( esc_html__( 'Get started with %s theme', 'newscrunch'  ), esc_html($theme_name)); ?></a></p>
			</div>
			<?php
		}
		...
		public function get_plugin_buttion($slug, $name) {
			...
			if (!$is_installed) {
    			...
    			{
    				$op_string=esc_html__('Install and activate', 'newscrunch' );
    			}
    			else
    			{
    				$op_string=esc_html__('Import Demo', 'newscrunch' );
    			}
				...
				$button_html        = sprintf('<a class="spicethemes-plugin-install install-now button-secondary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
					...
					/* translators: %s: theme name */
					sprintf(esc_html__('Install %s now', 'newscrunch' ), esc_html($name)),
					esc_html($name),
					$op_string
				);
			} 
			elseif ($is_installed && !$is_activeted) {
				...
				$button_html = sprintf('<a class="spicethemes-plugin-activate activate-now button-primary button" data-slug="%1$s" href="%2$s" aria-label="%3$s" data-name="%4$s">%5$s</a>',
					...
					/* translators: %s: theme name */
					sprintf(esc_html__('Activate %s now', 'newscrunch' ), esc_html($name)),
					esc_html($name),
					$activate_string
				);
			} 
		...
		{
			if( class_exists('Spice_Starter_Sites') && ($name=='Spice Starter Sites'))
			{
				return true;
			}	
...
function newscrunch_recommended_plugins_array($plugins){
		...
		$plugins[] = array(
			...
			'desc'     	=> esc_html__('To import the pre-designed demo data for the magazine theme, please install and activate this plugin.', 'newscrunch' ),
		);
		$plugins[] = array(
			'name'     	=> 'Contact Form 7',
			'slug'     	=> 'contact-form-7',
